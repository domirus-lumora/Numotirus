...
P2PNode *p2p_create(uint16_t port) {
  ...
  // [Fix-3] ZRTP 相关字段初始化为 NULL/0（calloc 已做）。
  n->zrtp = NULL;
  n->on_sas_cb = NULL;
  n->on_result_cb = NULL;
...
// Start ZRTP key exchange asynchronously. 异步启动 ZRTP 密钥交换。
int p2p_zrtp_start_exchange(P2PNode *n,
  ...
  // Create ZRTP session if needed. 按需创建 ZRTP 会话。
  if (!n->zrtp) {
  ...
  // Perform key exchange (non-blocking computation). 执行密钥交换（非阻塞计算）。
  if (zrtp_session_key_exchange(n->zrtp) != ZRTP_SUCCESS) {
    printf("ZRTP key exchange failed.\n");
    return -1;
  }

  // Store callbacks and user data. 存储回调和用户数据。
  n->on_sas_cb = on_sas;
  n->on_result_cb = on_result;
  n->zrtp_user_data = user_data;
  n->zrtp_exchanging = 1;

  // Get SAS and invoke callback. 获取 SAS 并调用回调。
...
// Confirm or reject the ZRTP session. 确认或拒绝 ZRTP 会话。
void p2p_zrtp_confirm(P2PNode *n, int confirmed) {
  ...
  // Invoke result callback if provided. 若提供了结果回调则调用。
  if (n->on_result_cb) {
    n->on_result_cb(confirmed, n->zrtp_user_data);
  }

  // Clear exchange flag but keep session for reuse. 清除交换标志，保留会话以便重用。
  n->zrtp_exchanging = 0;
}
