...
// STUN client for NAT traversal with symmetric NAT detection.
// STUN 客户端，用于 NAT 穿透，支持对称型 NAT 检测。
...
constexpr uint16_t kStunDefaultPort = 3478;
...
// Mapped address from STUN response.
// STUN 响应中的映射地址。
...
// STUN client with multiple-query support.
// STUN 客户端，支持多次查询。
class StunClient {
public:
    StunClient();
    ...
    std::optional<MappedAddress> QueryPublicAddress(
        ...
        uint16_t server_port = kStunDefaultPort,
