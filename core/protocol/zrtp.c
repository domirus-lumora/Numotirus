...
// ZRTP session implementation using libsodium crypto_kx.
// 使用 libsodium crypto_kx 的 ZRTP 会话实现。
...
// Compare two buffers of length 32 (public keys or keys).
// 比较两个长度为 32 的缓冲区（公钥或密钥）。
...
zrtp_error_t zrtp_session_key_exchange(zrtp_session_t* sess) {
  ...
  if (ret != 0) {
    return ZRTP_ERROR_KEY_EXCHANGE_FAILED;
  }

  // Sort rx and tx lexicographically before combining.
  // 将 rx 和 tx 按字典序排序后再组合，保证双方得到相同的字节序列。
  uint8_t combined[64];
...
// Constant trust store file name. 信任存储文件名。
static const char* kTrustStoreFile = "trusted_peers.bin";

// Simple trust store using binary file (overwrite mode for each save).
// 使用二进制文件的简单信任存储（每次保存为覆盖模式，避免追加）。
// Format: peer_id(32) + pubkey(32) + secret(32).
// 格式：peer_id(32) + 公钥(32) + 秘密(32)。
zrtp_error_t zrtp_trust_store_save(const char* peer_id,
  ...
  FILE* f = fopen(kTrustStoreFile, "wb");
...
zrtp_error_t zrtp_trust_store_load(const char* peer_id,
  ...
  char id[32];
  zrtp_error_t err = ZRTP_ERROR_TRUST_STORE_IO;  // Default to not found.
  while (fread(id, 1, 32, f) == 32) {
    ...
    // Skip pubkey + secret for non-matching entries.
    // 跳过不匹配条目的公钥和秘密。
    fseek(f, 64, SEEK_CUR);
  }
  fclose(f);
