// core/p2p/crypto_c.c
// ECIES encryption using libsodium. 使用 libsodium 的 ECIES 加密实现。

#include "crypto_c.h"
#include <sodium.h>
#include <stdlib.h>
#include <string.h>
#include <stdio.h>

#define PUBLIC_KEY_SIZE 32   // 公钥大小 / Public key size
#define SECRET_KEY_SIZE 32   // 私钥大小 / Secret key size
#define SHARED_SECRET_SIZE 32 // 共享秘密大小 / Shared secret size
#define SYMMETRIC_KEY_SIZE 32 // 对称密钥大小 / Symmetric key size
#define NONCE_SIZE 24        // 随机数大小 / Nonce size
#define TAG_SIZE 16          // 认证标签大小 / Authentication tag size

// Keypair structure. 密钥对结构。
struct CryptoKeypair {
    uint8_t public_key[PUBLIC_KEY_SIZE];
    uint8_t secret_key[SECRET_KEY_SIZE];
};

// Derive symmetric key from shared secret. 从共享秘密派生对称密钥。
static void derive_key(const uint8_t* shared, uint8_t* key) {
    crypto_generichash(key, SYMMETRIC_KEY_SIZE, shared, SHARED_SECRET_SIZE, NULL, 0);
}

// Generate a new keypair. 生成新的密钥对。
CryptoKeypair* crypto_keypair_generate(void) {
    if (sodium_init() < 0) return NULL;
    CryptoKeypair* kp = malloc(sizeof(CryptoKeypair));
    if (!kp) return NULL;
    crypto_box_keypair(kp->public_key, kp->secret_key);
    return kp;
}

// Free a keypair. 释放密钥对。
void crypto_keypair_free(CryptoKeypair* kp) {
    free(kp);
}

// Get public key. 获取公钥。
const uint8_t* crypto_keypair_get_public(const CryptoKeypair* kp) {
    return kp->public_key;
}

// Get secret key. 获取私钥。
const uint8_t* crypto_keypair_get_secret(const CryptoKeypair* kp) {
    return kp->secret_key;
}

// Encrypt plaintext with recipient's public key. 使用对方公钥加密明文。
int crypto_encrypt_public(const uint8_t* plain, size_t plain_len,
                          const uint8_t* pubkey,
                          uint8_t** out, size_t* out_len) {
    // Generate ephemeral key pair. 生成临时密钥对。
    uint8_t eph_pub[PUBLIC_KEY_SIZE];
    uint8_t eph_sec[SECRET_KEY_SIZE];
    crypto_box_keypair(eph_pub, eph_sec);

    // Compute shared secret. 计算共享秘密。
    uint8_t shared[SHARED_SECRET_SIZE];
    if (crypto_scalarmult_curve25519(shared, eph_sec, pubkey) != 0)
        return -1;

    // Derive symmetric key. 派生对称密钥。
    uint8_t key[SYMMETRIC_KEY_SIZE];
    derive_key(shared, key);

    // Generate random nonce. 生成随机 nonce。
    uint8_t nonce[NONCE_SIZE];
    randombytes_buf(nonce, NONCE_SIZE);

    // Encrypt. 加密。
    uint8_t* cipher = malloc(plain_len + TAG_SIZE);
    if (!cipher) return -1;

    unsigned long long clen;
    crypto_aead_xchacha20poly1305_ietf_encrypt(
        cipher, &clen,
        plain, plain_len,
        NULL, 0,
        NULL, nonce, key
    );

    // Assemble: ephemeral_public (32) + nonce (24) + ciphertext. 组装密文。
    *out_len = PUBLIC_KEY_SIZE + NONCE_SIZE + clen;
    *out = malloc(*out_len);
    if (!*out) {
        free(cipher);
        return -1;
    }

    uint8_t* p = *out;
    memcpy(p, eph_pub, PUBLIC_KEY_SIZE); p += PUBLIC_KEY_SIZE;
    memcpy(p, nonce, NONCE_SIZE); p += NONCE_SIZE;
    memcpy(p, cipher, clen);

    free(cipher);
    return 0;
}

// Decrypt ciphertext with own secret key. 使用自己的私钥解密密文。
int crypto_decrypt_private(const uint8_t* cipher, size_t cipher_len,
                           const uint8_t* seckey,
                           uint8_t** out, size_t* out_len) {
    // Check minimum length. 检查最小长度。
    if (cipher_len < PUBLIC_KEY_SIZE + NONCE_SIZE + TAG_SIZE)
        return -1;

    // Extract ephemeral public key, nonce, and ciphertext. 提取临时公钥、随机数和密文。
    const uint8_t* eph_pub = cipher;
    const uint8_t* nonce = cipher + PUBLIC_KEY_SIZE;
    const uint8_t* data = cipher + PUBLIC_KEY_SIZE + NONCE_SIZE;
    size_t data_len = cipher_len - PUBLIC_KEY_SIZE - NONCE_SIZE;

    // Compute shared secret. 计算共享秘密。
    uint8_t shared[SHARED_SECRET_SIZE];
    if (crypto_scalarmult_curve25519(shared, seckey, eph_pub) != 0)
        return -1;

    // Derive symmetric key. 派生对称密钥。
    uint8_t key[SYMMETRIC_KEY_SIZE];
    derive_key(shared, key);

    // Decrypt. 解密。
    uint8_t* plain = malloc(data_len);
    if (!plain) return -1;

    unsigned long long plen;
    if (crypto_aead_xchacha20poly1305_ietf_decrypt(
        plain, &plen,
        NULL,
        data, data_len,
        NULL, 0,
        nonce, key
    ) != 0) {
        free(plain);
        return -1;
    }

    *out = plain;
    *out_len = plen;
    return 0;
}